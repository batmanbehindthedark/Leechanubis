...
async def incoming_message_f(client, message):
    ...
    i_m_sefg = await message.reply_text("Avundi Agu Jara...", quote=True)
...
async def incoming_youtube_dl_f(client, message):
    ...
    i_m_sefg = await message.reply_text("<code>Uchapuko...🔃</code>", quote=True)
    # LOGGER.info(message)
    ...
    if message.reply_to_message:
        ...
        await i_m_sefg.edit("<b>🐈 Ayyooo Reply To YTDL Supported Link.</b>")
        return
    if dl_url is not None:
...
async def g_yt_playlist(client, message):
    ...
    if "youtube.com/playlist" in url:
        ...
        i_m_sefg = await message.reply_text(
            f"<b>Sare Manchindi 🧖 {u_men} Bro!!:\n Your Request has been ADDED</b>\n\n <code> Upload iyenta varaku Agu</code>",
            parse_mode="html",
        )
...
async def rename_tg_file(client, message):
    ...
    if len(message.command) > 1:
        ...
        try:
            ...
            if message_to_send != "":
                mention_req_user = (
                    f"<a href='tg://user?id={usr_id}'>🐯 Hey Mowa!! Your Requested Files 👇</a>\n\n"
                )
                message_to_send = mention_req_user + message_to_send
                message_to_send = message_to_send + "\n\n" + "<b> #UPLOADS\n\n💫 Powered By : @Madara_ucchiha</b>"
            else:
        ...
        await message.reply_text(
            "<b> Ayyo.. 😬</b>\n\nProvide Name with extension\n\n➩<b>Example</b>: <code> /rename ANUBIS.mkv</code>", quote=True
        )
